...
import UIKit
extension SendLixiVC {
    func initUI(){
        self.keyboardHelper = KeyboardHelper(viewController: self, scrollView: scrollView)
        self.keyboardHelper?.setDismissKeyboardWhenTouchOutside()
        view.fixInputAssistant()
        self.lbTitle.text = LanguageHelper.getTranslationByKey(LanguageKey.SendLixi)?.uppercased()
        ...
        self.lbChooseLixi.text = LanguageHelper.getTranslationByKey(LanguageKey.ChooseTemplate)
        self.lbChooseLixi.font = AppFonts.moderateScale(fontName: AppFonts.SFranciscoRegular, size: AppFonts.TEXTFIELD_TITLE_FONT_SIZE - 4)
        self.lbChooseLixi.textColor = AppColors.GRAY_LIGHT_TEXT
    }
    ...
    func bindingViewModel() {
        ...
        viewModel.errorMessages.bind({ [weak self] errors in
            if errors.count > 0 {
                self?.textFieldeCashId.errorMessage = errors[SendLixiViewModelListKey.ECASHID] ?? ""
                self?.textFieldContent.errorMessage = errors[SendLixiViewModelListKey.CONTENT] ?? ""
        ...
        viewModel.responseToView = {[weak self] value in
            ...
            else if value == EnumResponseToView.UPDATE_DATA_SOURCE_COLLECTION_VIEW.rawValue {
        ...
        self.viewModel.errorMessages.value[SendLixiViewModelListKey.ECASHID] = ""
        self.viewModel.errorMessages.value[SendLixiViewModelListKey.CONTENT] = ""
        self.viewModel.doBindingDataToView()
        self.viewModel.doGeteDongInfo()
        self.viewModel.getListAvailable()
        self.viewModel.getListTemplate()
    ...
    func bindTableView() {
        self.dataSource = TableViewDataSource(cellIdentifier: EnumIdentifier.SendLixi.rawValue, items: self.viewModel.listAvailable,isSelectionStype: false){ cell, vm in
    ...
    func setupTableView(){
        ...
        tableView.register(TableViewCell.self, forCellReuseIdentifier: EnumIdentifier.SendLixi.rawValue)
    ...
    func updateCollectionViewDataSource() {
          self.collectionViewDataSource.items = self.viewModel.listTemplate
    ...
    func bindCollectionView(){
        ...
        self.collectionViewDataSource = CollectionViewDataSource(cellIdentifier: EnumIdentifier.PaymentServices.rawValue,size: 100, items: self.viewModel.listTemplate){ cell , vm in
              cell.delegate = self
              cell.configView(view: vm)
        }
...
extension SendLixiVC: UITextFieldDelegate {
...
extension SendLixiVC : TableViewCellDelegate {
...
extension SendLixiVC: SingleButtonDialogPresenter { }

extension SendLixiVC : CollectionViewCellDelegate{
